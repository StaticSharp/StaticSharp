namespace CsmlGenerator {
    partial class Builder {
        const string α = "α";

        const string CsmlRoot = "CsmlRoot";
        const string ProtoNode = "ProtoNode";
        const string VirtualNodePropertyName = "VirtualNode";
        const string NodePropertyName = "Node";
        const string INode = "CsmlEngine.INode";
        const string Representative = "Representative";
        const string Representatives = "Representatives";
        const string IRepresentative = "CsmlEngine.IRepresentative";
        const string ITypedRepresentativeProvider = "CsmlEngine.ITypedRepresentativeProvider";
    }
}