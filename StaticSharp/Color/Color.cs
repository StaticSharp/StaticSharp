...
/*
public partial struct ColorA {
    public Color RGB { get; init; }
    public double A { get; init; } = 1;

    public ColorA(Color rgb, double a = 1) {
        RGB = rgb;
        A = a;
    }

    public static implicit operator ColorA(Color color) {
        return FromIntRGBA(color, 1);
    }
}

public partial struct ColorA : IConvertableToJsValue {
    public string ToJsValue() {
        return $"FromIntRGBA({RGB},{A})";
    }
}*/
...
public partial struct Color : IConvertableToJsValue , IKeyProvider {
    ...
    public string ToJsValue() {
        ...
        if (double.IsNaN(A))
            return $"new Color({R.ToInvariant()},{G.ToInvariant()},{B.ToInvariant()})";
        else
            return $"new Color({R.ToInvariant()},{G.ToInvariant()},{B.ToInvariant()},{A.ToInvariant()})";
    }
}
...
public partial struct Color {
    ...
    private static double unNaN(double value, double replacement) {
        if (double.IsNaN(value))
            return replacement;
        return value;
    }

    [ConvertToJs("Color.add({0},{1})")]
    public static Color operator + (Color a, Color b) {
        return new Color(a.R + b.R, a.G + b.G, a.B + b.B, unNaN(a.A,0) + unNaN(b.A,0));
    }
